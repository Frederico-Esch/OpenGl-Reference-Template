...
void load_glad(GLFWwindow *window) {
  int major_minor = gladLoadGLLoader((GLADloadproc)glfwGetProcAddress);
  if (major_minor == 0)
    exit_error("COULDN'T LOAD GL CORRECTLY\n", window);
#ifdef DEBUG_INFO
...
void generate_buffers(GLFWwindow *window, Buffers_struct *buffers) {
  if (buffers->vao_size > 0) {
    ...
    if (buffers->vao == NULL)
      exit_error("Error Creating VAO", window);
  ...
  if (buffers->vbo_size > 0) {
    ...
    if (buffers->vbo == NULL)
      exit_error("Error Creating VBO", window);
  ...
  if (buffers->ebo_size > 0) {
    ...
    if (buffers->ebo == NULL)
      exit_error("Error Creating EBO", window);
...
void load_shaders(GLFWwindow *window, GLuint *prog) {
  ...
  *prog = glCreateProgram();
  ...
  hasCompiled = GL_FALSE;
  tried = 0;
  while (!hasCompiled && tried < RETRY_COMPILATION_SHADERS) {
    const char *fragment_shader_text =
        read_shader(window, FRAGMENT_SHADER_PATH);
...
void background_flag(GLuint vbo, GLuint ebo) {
  ...
  glNamedBufferStorage(ebo, 6 * sizeof(GLuint), indexes,
                       GL_DYNAMIC_STORAGE_BIT);
  // glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6*sizeof(GLuint), indexes,
  // GL_STATIC_READ); unbound_buffers();
}
...
void diamond_flag(GLuint vbo, GLuint ebo) {
  Vertex data[] = {
      (Vertex){
          .point = (Point){.X = -1, .Y = 0},
          .color =
              (Color){.R = 255.f / 255.f, .G = 223.f / 255.f, .B = 0.f / 255.f},
          .texCoord = (Point){.X = 0, .Y = 0}},
      (Vertex){
          .point = (Point){.X = 0, .Y = 1},
          .color =
...
void circle_flag(GLuint vbo, GLuint ebo, int vertexes) {
  ...
  for (int i = 1; i < vertexes; i++) {
    ...
    data[i] = (Vertex){
        .point = (Point){.X = (GLfloat)(radius * cos(angle)),
  ...
  // glBufferData(GL_ARRAY_BUFFER, vertexes*sizeof(Vertex), data,
  // GL_STREAM_DRAW);

  glNamedBufferData(ebo, amount_indexes * sizeof(GLuint), indexes,
                    GL_STREAM_DRAW);
  // glBufferData(GL_ELEMENT_ARRAY_BUFFER, amount_indexes*sizeof(GLuint),
  // indexes ,GL_STREAM_DRAW);

  free(indexes);
  free(data);
}

void bind_buffer(Buffers_struct *buffers, size_t vao_index, size_t vbo_index,
                 size_t ebo_index) {
...
void initialize(Window_struct *window, GLFWerrorfun _error_callback,
                GLFWkeyfun _key_callback, GLFWwindowsizefun _resize_callback) {
  if (!glfwInit())
    exit_error("Error loading glfw", window->window);
...
void cleanup(State *main_state) {
  glDeleteTextures(main_state->textures->texture_size,
                   main_state->textures->texture);
...
void setup_attribs(Buffers_struct *buffers) {
  for (int i = 0; i < buffers->vao_size; i++) {
    ...
    glVertexArrayAttribFormat(buffers->vao[i], VA_ATT_POS, 2, GL_FLOAT,
                              GL_FALSE, 0); // No stride?? Strange
    glVertexArrayAttribFormat(buffers->vao[i], VA_ATT_COL, 3, GL_FLOAT,
                              GL_FALSE, sizeof(Point)); // No stride?? Strange
    glVertexArrayAttribFormat(
        buffers->vao[i], VA_ATT_TEX, 2, GL_FLOAT, GL_FALSE,
        sizeof(Color) + sizeof(Point)); // No stride?? Strange
    // glVertexAttribPointer(VA_ATT_POS, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex),
    // NULL); glVertexAttribPointer(VA_ATT_COL, 3, GL_FLOAT, GL_FALSE,
    // sizeof(Vertex), (void*)sizeof(Point));

    glVertexArrayAttribBinding(buffers->vao[i], VA_ATT_POS,
                               0); // binding points dont make sense
    glVertexArrayAttribBinding(buffers->vao[i], VA_ATT_COL,
                               0); // binding points dont make sense
    glVertexArrayAttribBinding(buffers->vao[i], VA_ATT_TEX,
                               0); // binding points dont make sense
  }
}
...
void binding_buffers(Buffers_struct *buffers) {
  for (int i = 0; i < buffers->vao_size; i++) {
    glVertexArrayVertexBuffer(buffers->vao[i], 0, buffers->vbo[i], 0,
                              sizeof(Vertex));
...
void setup_textures(Texture_struct *textures) {
  textures->texture_info =
      malloc(textures->texture_size * sizeof(TextureInfo_struct));
...
void setup_texture_uniforms(Texture_struct *texture) {
  for (int index = 0; index < texture->texture_size; index++)
    glUniform1i(glGetUniformLocation(texture->texture_info[index].program,
                                     texture->texture_info[index].name),
                texture->texture_info[index].unit);
  glCreateTextures(GL_TEXTURE_2D, texture->texture_size, texture->texture);
}

void setup_texture_parameters(GLuint texture, GLenum *parameters,
                              GLuint *values, size_t parameter_count) {
  for (size_t i = 0; i < parameter_count; i++)
    glTextureParameteri(texture, parameters[i], values[i]);
}

void load_textures(GLuint texture, TextureInfo_struct *texture_info,
                   const char *path_to_texture) {
  unsigned char *data =
      stbi_load(path_to_texture, &texture_info->width, &texture_info->height,
                &texture_info->channels, STBI_rgb_alpha);
  int ok = stbi_info(path_to_texture, &texture_info->width,
                     &texture_info->height, &texture_info->channels);
  printf_s("Image path=%s, ok=%d, w=%d, h=%d\n", path_to_texture, ok,
           texture_info->width, texture_info->height);
  glTextureStorage2D(texture, 1, GL_RGBA8, texture_info->width,
                     texture_info->height);
  glTextureSubImage2D(texture, 0, 0, 0, texture_info->width,
                      texture_info->height, GL_RGBA, GL_UNSIGNED_BYTE, data);

  setup_texture_parameters(
      texture,
      (GLenum[]){GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, GL_TEXTURE_MIN_FILTER,
                 GL_TEXTURE_MAG_FILTER},
      (GLuint[]){GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_LINEAR}, 4);
...
void enableTexture(GLuint program, GLuint texture,
                   TextureInfo_struct *texture_info) {
  glUniform1i(glGetUniformLocation(program, "whichTexture"),
              texture_info->unit);
...
int main() {
  ...
  main_state.buffers = &buffers;
  Uniforms uniforms;
  main_state.uniforms = &uniforms;
  Texture_struct textures;
  main_state.textures = &textures;
  main_state.revise_circle = 0;
  main_state.reload_shaders = 0;
  main_state.to_render = BACKGROUND;

  window = (Window_struct){.Width = 600, .Height = 400, .Title = "Principal"};

  initialize(&window, error_callback, key_callback, window_resize_callback);
  ...
  buffers.program_size = 1;
  buffers.program = malloc(buffers.program_size * sizeof(GLuint));
  ...
  textures.texture_info[0] = (TextureInfo_struct){
      .name = "fabricTexture", .unit = 0, .program = buffers.program[0]};

  setup_texture_uniforms(&textures);
  load_textures(textures.texture[0], &textures.texture_info[0],
                FABRIC_TEXTURE_PATH);
  ...
  while (!glfwWindowShouldClose(window.window)) {
    ...
    if ((toBeRendered & BACKGROUND) == BACKGROUND) {
      enableTexture(buffers.program[0], textures.texture[0],
                    &textures.texture_info[0]);
    ...
    if ((toBeRendered & CIRCLE) == CIRCLE) {
      ...
      glDrawElements(GL_TRIANGLE_FAN, circle_vertexes + 1, GL_UNSIGNED_INT,
                     NULL);
    }
    glBindVertexArray(0);
